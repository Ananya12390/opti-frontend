...
export default function MyGearPage() {
  ...
  useEffect(() => {
    api.assets.list().then(a => setAssets(a.filter(x => x.assigned_to === user?.id))).finally(() => setLoading(false));
  }, [user]);
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6 animate-fade-in">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {assets.length === 0 ? "No assets assigned to you yet" : `${assets.length} item${assets.length === 1 ? "" : "s"} assigned to you`}
        </p>
      </div>
      ...
      {assets.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card/50 p-16 text-center animate-fade-in">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">Contact your manager to get assets assigned</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 animate-fade-in animation-delay-100">
          {assets.map(a => (
            <div key={a.id} className="rounded-2xl border border-border bg-card p-5 hover:border-primary/30 transition-all">
              <div className="flex items-start justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center text-primary">
                  <Laptop className="w-6 h-6" />
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${STATUS_CLASS[a.status]}`}>{a.status}</span>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${CONDITION_CLASS[a.condition]}`}>{a.condition}</span>
                </div>
              </div>
              <h3 className="font-semibold text-foreground mb-1">{a.name}</h3>
              <p className="text-xs text-muted-foreground font-mono mb-3">{a.serial_number}</p>
              <div className="space-y-1.5 pt-3 border-t border-border">
                ...
                {a.purchase_date && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span>Purchased {new Date(a.purchase_date).toLocaleDateString("en-GB", { year: "numeric", month: "short", day: "numeric" })}</span>
                  </div>
                )}
                {a.notes && (
